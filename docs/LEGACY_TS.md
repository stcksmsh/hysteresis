# LEGACY_TS.md — the browser/TypeScript system (frozen reference, archived from AGENTS.md)

> **FROZEN.** This is the TypeScript/browser `sinteza-viz` implementation, archived verbatim from
> `AGENTS.md` the session the Rust rewrite (`SINTEZA_IMPLEMENTATION_PLAN.md`) began. Per that
> plan's §0.2: kept running, not maintained, not backported, not half-migrated — its only value
> now is behavioural diffing ("does the Rust memory field look like the browser one"). Deleted in
> one commit when the Rust rewrite supersedes it. **Never edit the TS tree** (plan §1.2) — read
> freely, port from it, that's all. Current work lives in `AGENTS.md` at the repo root; this file
> is background only.
